...
from health_website.utils import getDropDown, dictfetchall
...
def user_listing(request, userId):
    ...
    if(userId == "2"):
        context['heading'] = "Patients Report"
    if(userId == "3"):
        context['heading'] = "Doctor Report"
...
def update(request, userId):
    ...
    if(currentUserDetails.user_level_id == 2):
        context['heading'] = "Patients";
    if(currentUserDetails.user_level_id == 2):
        context['heading'] = "Doctor";
...
def add(request):
    ...
    "fn":"add",
    "citylist":city.objects.all(),
    "rolelist": getDropDown('users_role', '1'),
    "heading":'Users',
    "sub_heading": 'Users',
    "statelist":state.objects.all(),
    "countrylist":country.objects.all()
    ...
    if(request.session.get('authenticated', False) != True):
        context["rolelist"] = getDropDown('users_role', 'role_visible = 1')
    if (request.method == "POST"):
        try:
            user_image = None
        ...
        if(request.session.get('authenticated', False) != True):
            messages.add_message(request, messages.INFO, "Your account has registered successfully. Login with your credentials !!!")
